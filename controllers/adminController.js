...
const adminController = {
  ...
  actionSignin: async (req, res) => {
    try {
      ...
      if (!user) {
        req.flash("alertMessage", "User yang anda masukan tidak ada!!");
        req.flash("alertStatus", "danger");
        return res.redirect("/admin/signin"); // Tambahkan 'return' di sini
      }
      const isPasswordMatch = await bcrypt.compare(password, user.password);
      if (!isPasswordMatch) {
        req.flash("alertMessage", "Password yang anda masukan tidak cocok!!");
      ...
      return res.redirect("/admin/dashboard"); // Tambahkan 'return' di sini
    } catch (error) {
      return res.redirect("/admin/signin"); // Tambahkan 'return' di sini
    }
  },
  ...
  //sekaligus mendelete di direktory
  deleteBank: async (req, res) => {
    try {
      const { id } = req.params;
      const bank = await Bank.findOne({ _id: id });
      if (bank) {
        ...
        await fs.promises.unlink(imagePath); // Menggunakan fs.promises untuk penghapusan
        await bank.deleteOne();
        req.flash("alertMessage", "Success Delete Bank");
        req.flash("alertStatus", "success");
      } else {
